...
import { BackButtonIcon, CurrentLocation, DeleteIcon,} from "../../assets/Icons";
...
const AddFieldMap = ({ setMarkers, markers }) => {
    ...
    const SearchField = ({ onLocationSelect }) => {
        ...
        const searchControl = new GeoSearchControl({
            ...
            form: "w-full",
            input: " w-full p-2 border-b border-gray-300 text-sm text-black bg-white",
            results: "z-[2000] bg-white border border-gray-300 rounded max-h-[200px] overflow-y-auto w-full",
            result: "p-2 cursor-pointer text-gray-800 text-sm hover:bg-gray-100 hover:text-black transition ease-in-out duration-400",
            },
        ...
        <div className="min-h-screen w-full flex flex-col items-center relative overflow-hidden">
            <div className="h-screen w-full m-0 p-0 z-[1000] relative">
                ...
                {selectedLocation?.lat && selectedLocation?.lng ? (
                    <>
                        <MapContainer center={[selectedLocation.lat, selectedLocation.lng]}
                                        zoom={17}
                                        zoomControl={true}
                            ...
                            {markers.length > 0 && (
                                <Polygon positions={markers.map((marker) => [marker.lat, marker.lng])} color="yellow" />
                            )}
                        ...
                        <div className="absolute top-0 right-2 h-screen flex flex-col justify-center gap-4 z-[1000] ">
                            ...
                            <button onClick={() => {
                                    ...
                                    className={`bg-[#075a53] text-white w-10 h-10 rounded-full flex items-center justify-center transition duration-400 ease-in-out cursor-pointer hover:bg-[#064841] ${selectedIcon == "remove" ? "selected-icon" : ""}`} >
                                    <DeleteIcon />
                            </button>
                        ...
                        <div className="absolute bottom-1 left-1/2 transform -translate-x-1/2 z-[1100] w-[95%] md:max-w-[500px] lg:max-w-[900px]  bg-[#5a7c6b] rounded shadow-md mx-auto">
                            <div className="w-full text-center z-[1000]">
                                <div className="flex justify-between items-center gap-4 p-2.5 bg-[#5a7c6b] rounded w-full">
                                    ...
                                    <button className="px-4 text-sm lg:text-base rounded bg-[#5a7c6b] text-white font-semibold whitespace-nowrap"
