...
const Signup = ({ setActiveTab }) => {
  ...
  const validate = () => {
    ...
    if (!formData.email || !emailRegex.test(formData.email)) return alert("Invalid email.");
    if (!formData.phone || !phoneRegex.test(formData.phone)) return alert("Phone must be 10 digits.");
  ...
  return (
    <form
      ...
      className="w-full max-w-md space-y-5 px-4 sm:px-0 mt-6 sm:mt-12"
    >
      {/* Heading */}
      <div>
        <h2 className="text-2xl font-semibold text-gray-900">Get started with CropGen</h2>
        <p className="text-sm text-gray-600">Enter your personal data to create your account.</p>
      </div>
      ...
      <div className="flex items-start gap-2 text-sm">
        ...
        <label htmlFor="terms" className="text-gray-700">
          I agree to the{" "}
          <a href="#" className="text-green-600 underline">Terms</a> and{" "}
