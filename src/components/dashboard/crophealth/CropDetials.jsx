  ...
  return (
    <div className="flex flex-col sm:flex-row items-center mb-4 md:mb-0 md:gap-2 md:scale-[0.85] md:pl-1 lg:scale-100 lg:pl-4">
      <img
        src={cropImage}
        alt="crop image"
        className="w-24 h-24 sm:w-32 sm:h-32 md:w-20 md:h-20 lg:w-32 lg:h-32 border-2 border-green-600 rounded-md p-1 object-cover"
      />
      <div className="mt-4 sm:mt-0 sm:ml-4 md:ml-2 flex flex-col gap-2 md:gap-1">
        {[
          ...
          <div key={i} className="flex items-center">
            <span className="text-sm sm:text-base md:text-xs lg:text-base font-semibold text-[#344E41] min-w-[120px]">
              {label}:
            </span>
            <span className="text-sm sm:text-base md:text-xs lg:text-base text-black">{value}</span>
          </div>
        ))}
      </div>
    </div>
